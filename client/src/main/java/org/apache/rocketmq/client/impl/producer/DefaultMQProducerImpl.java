...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    private SendResult sendDefaultImpl(
        ...
        this.makeSureStateOK();     // 确定producer服务器状态正常
        Validators.checkMessage(msg, this.defaultMQProducer);
        final long invokeID = random.nextLong();    // invokeID的作用？
        long beginTimestampFirst = System.currentTimeMillis();
        long beginTimestampPrev = beginTimestampFirst;
        long endTimestamp = beginTimestampFirst;
        TopicPublishInfo topicPublishInfo = this.tryToFindTopicPublishInfo(msg.getTopic()); //获取topic的路由信息
        if (topicPublishInfo != null && topicPublishInfo.ok()) {
            ...
            int times = 0;
            String[] brokersSent = new String[timesTotal];  // 保存每次重试的brokerName
            for (; times < timesTotal; times++) {
                String lastBrokerName = null == mq ? null : mq.getBrokerName();
                MessageQueue mqSelected = this.selectOneMessageQueue(topicPublishInfo, lastBrokerName); // 选取一个messageQueue
                if (mqSelected != null) {
                    mq = mqSelected;
                    brokersSent[times] = mq.getBrokerName();
                    try {
                        beginTimestampPrev = System.currentTimeMillis();    // 已经找到发送目标，准备发送消息的系统时间
                        if (times > 0) {
                            //Reset topic with namespace during resend.
                            msg.setTopic(this.defaultMQProducer.withNamespace(msg.getTopic()));
                        }
                        long costTime = beginTimestampPrev - beginTimestampFirst;   // 找路由及messageQueue花费的时间
                        ...
                        endTimestamp = System.currentTimeMillis();  // 发送完消息的时间
